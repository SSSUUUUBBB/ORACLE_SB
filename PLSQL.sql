...
BEGIN            
    ...
    DBMS_OUTPUT.PUT_LINE('D1 부서 최대 급여 : ' || vn_salary);
...
BEGIN                   
    FOR i IN 1..20
    LOOP
        CONTINUE WHEN MOD(i, 2) = 0;   --MOD() : 나머지함수
...
BEGIN                   
    CASE
        WHEN vn_var = 'A' THEN DBMS_OUTPUT.PUT_LINE('A');
        WHEN vn_var = 'B' THEN DBMS_OUTPUT.PUT_LINE('B');
        ELSE NULL;      --아무것도 처리하지 않는 NULL문
    END CASE;
END;
/
...
IS
    ...
    --%ROWTYPE : 해당 테이블 또는 뷰 컬럼들을 참조타입으로 선언
BEGIN
    SELECT * INTO V_EMP
    FROM employee
    WHERE emp_id = IN_EMP_ID;
...
BEGIN
    ...
    OUT_RESULT_STR := V_EMP.dept_code;
...
EXECUTE pro_out_mul(1, 2, 3) --OUT파라미터가 있어서, 블록 안에서 호출해야함
DECLARE
